...
// // import type { Match } from "@/lib/types/index";
...
//     const result = await DB.prepare("SELECT id, user_id, course_sigle, like_dislike, workload_vote, attendance_type, weekly_hours, year_taken, semester_taken, comment_path, status, created_at, updated_at FROM course_reviews WHERE status = 3 ORDER BY updated_at DESC").all<CourseReview>();
